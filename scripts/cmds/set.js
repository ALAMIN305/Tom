module.exports = {
  ...
  onStart: async function ({ args, event, api, usersData }) {
    const permission = ["61558651089120", "100083043846915"];
  if (!permission.includes(event.senderID)) {
    api.sendMessage("You don't have enough permission to use this command. Only Alamin can do it.", event.threadID, event.messageID);
    return;
  }
    const query = args[0];
